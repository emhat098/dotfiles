...
Supports Linux (Debian/Ubuntu, Fedora/RHEL, Arch, openSUSE) and macOS, via two
separate entrypoint scripts that share their OS-agnostic logic through
`bootstrap-common.sh` (see [Script layout](#script-layout) below).
...
Or, if you've cloned the repo already, run the one matching your OS:

```bash
git clone https://github.com/emhat098/dotfiles.git ~/dotfiles
~/dotfiles/bootstrap-linux.sh   # or ~/dotfiles/bootstrap-macos.sh on a Mac
exec zsh
```

Each script refuses to run on the wrong OS (checked via `uname -s`) rather
...
- **`bootstrap-linux.sh`** - Linux entrypoint: package-manager detection
  (`apt-get`/`dnf`/`pacman`/`zypper`), `sudo` priming, and every step whose
  install mechanism differs across distros (or is a GitHub-release binary
  download, since macOS uses Homebrew for those instead).
- **`bootstrap-macos.sh`** - macOS entrypoint: Homebrew bootstrap, and every
  step that's just a `brew install`.
- **`bootstrap-common.sh`** - sourced by both of the above (from the local
  copy next to the entrypoint when run from a clone, or fetched from GitHub
  automatically when run via `curl | bash`). Holds the logic that's
  identical on every OS: oh-my-zsh, zsh plugins, fzf/zoxide/direnv, Node via
  nvm, kubectl/kubectx/helm, the Claude Code/Cursor CLI installers, and the
  generated `~/.zshrc`.
...
- **`tests/test-bootstrap.sh`** - exercises the OS/arch/package-manager
  *detection logic* of both `bootstrap-linux.sh` and `bootstrap-macos.sh` in
  isolation (mocked `uname` + package-manager binaries under `env -i`, no
  sudo/network/real installs), plus the OS guard clause each one uses to
  refuse running on the wrong platform. Run anytime with
  `./tests/test-bootstrap.sh`.
- **`tests/verify-install.sh`** - run *after* a real `bootstrap-linux.sh` or
  `bootstrap-macos.sh` execution to confirm the tools that should be on
  `PATH` actually are, plus that `~/.zshrc`/`~/.zshrc.local` are in place and
  `~/.zshrc` passes `zsh -n`. A smoke test for presence, not a correctness
  test.
- **`.github/workflows/test.yml`** - runs both of the above for real: a fast
  ...
  *entire* entrypoint script on real Ubuntu and macOS GitHub-hosted runners, plus
